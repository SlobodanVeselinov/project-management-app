<template>
    <div class="p-5 pb-12 min-h-[calc(100%_-_10rem)]">
        <Transition>
            <NewProjectModal v-if="showModal" @closeModal="closeModal" />
        </Transition>

        <div
            class="lg:flex items-center justify-between pb-4 bg-white p-3 mb-5"
        >
            <input
                type="text"
                v-model="search"
        ...
        <div
            v-else-if="user.projects.length"
            class="lg:grid lg:grid-cols-3 lg:gap-5"
        >
            <div
                v-for="project in filteredToUser"
                :key="project.id"
...
export default {
    ...
    computed: {
        ...
        filteredToUser() {
            return this.user.projects.filter((project) =>
                project.name
                    .toString()
                    .toLowerCase()
                    .includes(this.search.toLowerCase())
